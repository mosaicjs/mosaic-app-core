...
import { Intents, Singlethon } from 'mosaic-intents';
 ...
 * </ol>
 */
export default class Application extends AppRouter {
    ...
    _updateActiveSlot(newSlot) {
        return this.action('module', function(){
            const promise = super._updateActiveSlot(newSlot);
            return promise;
        });
    }
    ...
    setState(state){
        const that = this;
        if (!that._setState) {
            that._setState = Singlethon.singlethonAction(that, 'state', 
            function(intent){
                return Promise.resolve().then(function(){
                    for (let type in intent.params) {
    ...
    _initStateFields(){
        ...
        ['locale', ...stateFields].forEach(function(key){
            index[key] = true;
        });
